...
class MyDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, args,mode="train"):
        ...
        #将数据按每个用户和每个item分好
        self.init_all()
        #生成faiss库:self.faiss_index(emb，k=topk)以召回
        if self.mode=="train":
    ...
    def __getitem__(self, index):
        ...
        if self.mode == 'train':
            # 训练模式：Next item Loss
            # 如果序列太短(例如只有1个)，可能在 __init__ 里就已经过滤掉了 ~最短的序列为两个
            hist_article_ids = full_article_ids[:-1]
            pos_id = full_article_ids[-1] #for NS_Token/排序阶段/验证集
            pos_seq = full_article_ids[1:]  # 用于序列到序列的损失 (Next Item Loss) 

            
            # 提取对应的用户侧序列特征
            hist_time_diff_norm = user_data['time_diff_norm'][:-1]
            hist_time_diff = user_data['time_diff'][:-1]
            hist_click_env = user_data['click_environment'][:-1]
            hist_click_dev = user_data['click_deviceGroup'][:-1]
            hist_click_os = user_data['click_os_mapped'][:-1]
            hist_click_region = user_data['click_region_mapped'][:-1]
            hist_click_referrer = user_data['click_referrer_type'][:-1]

        else:
            # 测试/验证模式：用全部历史预测未来(未知的)下一个
            hist_article_ids = full_article_ids
            pos_id = 0 # 占位符
            pos_seq = np.zeros(0, dtype=np.int64)
        ...
        # 这些特征不需要像 user 特征那样切片，直接用 hist_article_ids 查找即可
        ...
        # 生成 mask: 真实交互为1，填充部分为0,NS_token为2
        #token_mask = np.zeros(max_len +1 , dtype=np.int32) #+1 for 1个NS_token 
        #if real_len > 0:
        #    token_mask[max_len - real_len : max_len] = 1
        #token_mask[-1] = 2
        #2到最后再填。
        ...
        data = {
            ...
            "pos_seq":np.append(self._pad_truncate(pos_seq, max_len, pad_val=0, dtype=np.int64),pos_id),
        ...
        # 4. 负采样 (仅训练时)
        if self.mode == 'train':
            # 简单随机负采样：随机选一个不在历史记录中的 item
            # 实际比赛中可能需要更复杂的负采样策略（如流行度负采样）；
            #目前使用：In-batch InfoNCE + 随机负采样
            ts = set(tuple(full_article_ids))
            ts.add(tuple(pos_seq))
            neg = np.zeros([max_len+1], dtype=np.int32) #+1 for NS_Token
            for i in range(max_len+1):
                neg_id = self.random_neg(ts) #  item id 从 0 开始
                #ts.add(neg_id) #通常允许重复？
                neg[i] = neg_id
            data['neg_seq'] = neg
        else:
            #测试模式用全 0 数组占位，保证 batch 堆叠时不报错
            data['neg_seq'] = np.zeros(max_len+1, dtype=np.int64)
    ...
    def sort_item(self,candidate_item):
        #排序时调用__getitem__，然后将候选item的id 、各属性拼接到最后面
        #如果排序阶段要构造新特征也在此构造(后续再补充)

        #如果需要的话：候选item的时间戳未知~用平均时间差来代替。拿到上一个时间戳 +平均时间差得到一个估计时间戳来计算sincos等时间特征
        return
    ...
    def _build_faiss_index(self):
            """
            根据 self.items_id_embed 构建 Faiss 索引，并保存索引到 article_id 的映射。
            """
            ...
            try:
                ...
                # 1. 提取 embedding 向量
                # 假设第一列是 'article_id'，其余列是 embedding
                vectors = self.items_id_embed.iloc[:, 1:].values
                
                # 2. 确保数据类型为 float32 且内存连续 (Faiss 要求)
                ...
                # 如果你的 embedding 已经归一化，使用 IndexFlatIP (内积) 会更合适，等价于余弦相似度
                ...
                # 6. [关键] 保存 Faiss 索引 (0, 1, 2...) 到 原始 article_id 的映射
                self.faiss_id_map = self.items_id_embed.iloc[:, 0].values.astype(np.int64)
                
                print(f"Faiss index built. Total items: {self.faiss_index.ntotal}, Dimension: {d}")

            except ImportError:
                print("="*50)
                print("Warning: 'faiss-cpu' or 'faiss-gpu' library not found.")
                print("Faiss index will not be built. Retrieval-based operations will fail.")
